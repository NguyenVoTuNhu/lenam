...
(function () {
  ...
  const TABLES = (typeof KIO_CONFIG !== 'undefined' && KIO_CONFIG.logisticsTables) || {
    vehicles:'lenam_logistics_vehicles', drivers:'lenam_logistics_drivers', deliveries:'lenam_logistics_deliveries', maintenance:'lenam_logistics_maintenance'
  };
  const COLLECTIONS = ['vehicles','drivers','deliveries','maintenance'];
  let syncTimer = null;
  ...
  const seed = () => ({
    vehicles: [
      { id:'XE-001', plate:'51C-123.45', type:'Xe tải lạnh', capacityKg:1500, fuelType:'Diesel', fuelNorm:11, odometer:48520, status:'AVAILABLE', gpsDevice:'GPS-001', nextMaintenanceDate:'2026-10-01', nextMaintenanceKm:50000, registrationExpiry:'2026-12-20', note:'' },
      { id:'XE-002', plate:'51C-678.90', type:'Xe tải thùng', capacityKg:1200, fuelType:'Diesel', fuelNorm:10, odometer:37640, status:'AVAILABLE', gpsDevice:'GPS-002', nextMaintenanceDate:'2026-09-28', nextMaintenanceKm:40000, registrationExpiry:'2027-02-15', note:'' },
      { id:'XE-003', plate:'50H-246.80', type:'Xe van lạnh', capacityKg:800, fuelType:'Xăng', fuelNorm:9, odometer:29850, status:'MAINTENANCE', gpsDevice:'GPS-003', nextMaintenanceDate:'2026-09-16', nextMaintenanceKm:30000, registrationExpiry:'2026-11-30', note:'Đang bảo trì định kỳ' }
    ],
    drivers: [
  ...
  function deliveryDetail(d) {
    ...
    return `<div class="grid g-3" style="margin-bottom:14px">${card('Trạng thái', (statusBadge(d.status)), 'fa-circle-info','blue')}${card('Quãng đường',`${num(d.actualKm,1)} / ${num(d.estimatedKm,1)} km`,'fa-road','teal')}${card('Chi phí/km',n(d.actualKm)?money(costPerKm(d)):'—','fa-coins','orange')}</div>
      <div class="form-grid cols-2"><div class="field"><label>Khách hàng</label><div class="inp">${e(d.customer)}</div></div><div class="field"><label>Hạn giao</label><div class="inp">${dateFmt(d.promisedDate)} ${e(d.promisedTime||'')}</div></div><div class="field"><label>Địa chỉ</label><div class="inp">${e(d.address)}</div></div><div class="field"><label>Người nhận / SĐT</label><div class="inp">${e(d.recipient||'—')}${d.phone?` · ${e(d.phone)}`:''}</div></div><div class="field"><label>Tham chiếu đơn bán</label><div>${orderRef}</div></div><div class="field"><label>Phí giao hàng khách trả</label><div class="inp">${money(d.shippingFee||0)}</div></div><div class="field"><label>Xe</label><div class="inp">${e(v?.plate||'Chưa điều phối')}</div></div><div class="field"><label>Tài xế</label><div class="inp">${e(dr?.name||'Chưa điều phối')}</div></div><div class="field"><label>Tuyến</label><div class="inp">${e(d.route||'—')}</div></div><div class="field"><label>Khối lượng vận chuyển</label><div class="inp">${n(d.cargoKg)>0?`${num(d.cargoKg,2)} kg`:'Chưa cập nhật'}</div></div>${d.deliveryNote?`<div class="field" style="grid-column:1/-1"><label>Ghi chú giao hàng</label><div class="inp">${e(d.deliveryNote)}</div></div>`:''}</div>
      ...
      <div class="form-sec-title">Chi phí & nhiên liệu</div><div class="grid g-auto-sm">${card('Nhiên liệu thực tế',`${num(d.fuelLiters,1)} L`,'fa-gas-pump','blue')}${card('Nhiên liệu định mức',`${num(norm,1)} L`,'fa-gauge','slate')}${card('Chênh lệch',`${variance>=0?'+':''}${num(variance,1)} L`,'fa-scale-balanced',variance>0?'orange':'green')}${card('Chi phí vận chuyển thực tế',money(tripCost(d)),'fa-money-bill','teal')}${card('Phí giao khách trả',money(d.shippingFee||0),'fa-receipt','indigo')}${card('Chênh lệch phí giao',money(n(d.shippingFee)-tripCost(d)),'fa-scale-balanced',n(d.shippingFee)-tripCost(d)>=0?'green':'orange')}</div>`;
  }
  ...
  function orderSnapshot(o) {
    ...
    return {
      ...
      items: (o.items || []).map(it => ({ productId:it.productId, name:it.name || (typeof Q!=='undefined'&&Q.product?Q.product(it.productId)?.name:'') || it.productId, unit:it.unit || '', orderedQty:n(it.qty), deliveredQty:0, returnedQty:0 }))
    };
  }
  function renderOrderPreview(orderId) {
    ...
    const customer = document.querySelector('#lgCustomer'), address = document.querySelector('#lgAddress'), pd = document.querySelector('#lgPromiseDate');
    if (customer) customer.value = snap.customer; if (address) address.value = snap.address; if (pd) pd.value = snap.promisedDate < today() ? today() : snap.promisedDate;
    box.innerHTML = `<div class="form-sec-title">Hàng giao theo đơn ${e(o.id)}</div>${table([{t:'Thành phẩm'},{t:'Số lượng',cls:'right'},{t:'ĐVT'}], snap.items.map(it=>`<tr><td>${e(it.name)}<div class="cell-sub">${e(it.productId)}</div></td><td class="right num">${num(it.orderedQty,2)}</td><td>${e(it.unit)}</td></tr>`).join(''), 'Đơn hàng không có sản phẩm')}`;
  }
  function openDeliveryForm() {
    load(); const t=today();
    Modal.open({title:'Tạo đơn giao hàng',sub:'Đơn giao được tạo trực tiếp từ Đơn hàng bán đã hoàn tất xuất kho.',size:'lg',body:`<div class="form-grid cols-2"><div class="field" style="grid-column:1/-1"><label>Đơn bán tham chiếu *</label><select class="inp" id="lgOrderId">${crmOrderOptions()}</select></div><div class="field"><label>Khách hàng</label><input class="inp" id="lgCustomer" readonly></div><div class="field"><label>Khối lượng vận chuyển (kg) *</label><input class="inp num" id="lgCargo" type="number" min="0.01" step="0.01" value="100"><small>Khối lượng dùng để kiểm tra tải trọng xe; số lượng hàng lấy trực tiếp từ đơn bán.</small></div><div class="field" style="grid-column:1/-1"><label>Địa chỉ giao</label><input class="inp" id="lgAddress" readonly></div><div class="field"><label>Ngày giao *</label><input class="inp" id="lgPromiseDate" type="date" min="${t}" value="${t}"></div><div class="field"><label>Giờ cam kết *</label><input class="inp" id="lgPromiseTime" type="time" value="10:00"></div><div class="field"><label>Km dự kiến</label><input class="inp num" id="lgEstKm" type="number" min="0" step="0.1" value="20"></div><div class="field"><label>Ghi chú</label><input class="inp" id="lgNote"></div><div id="lgOrderPreview" style="grid-column:1/-1"><div class="note-box">Chọn đơn hàng đã được Kho xuất thành phẩm để tạo chuyến giao.</div></div></div>`,foot:'<button class="btn" data-act="modal-close">Hủy</button><button class="btn btn-primary" data-act="lg-delivery-save"><i class="fa-solid fa-floppy-disk"></i>Lưu đơn giao</button>'});
  ...
  function openDispatch(d) {
    const availVehicles=L.vehicles.filter(v=>v.status==='AVAILABLE'); const availDrivers=L.drivers.filter(x=>x.status==='AVAILABLE');
    Modal.open({title:`Điều phối · ${d.id}`,sub:`${d.customer} · ${n(d.cargoKg)>0?`${num(d.cargoKg)} kg`:'chưa nhập khối lượng'}`,size:'md',body:`<div class="form-grid cols-2"><div class="field"><label>Khối lượng vận chuyển (kg) *</label><input class="inp num" id="lgDispatchCargo" type="number" min="0.01" step="0.01" value="${n(d.cargoKg)>0?n(d.cargoKg):''}" placeholder="Nhập khối lượng thực tế"></div><div class="field"><label>Xe *</label><select class="inp" id="lgVehicle"><option value="">-- Chọn xe --</option>${availVehicles.map(v=>`<option value="${e(v.id)}">${e(v.plate)} · tải ${num(v.capacityKg)} kg · ĐM ${num(v.fuelNorm,1)} L/100km</option>`).join('')}</select></div><div class="field"><label>Tài xế *</label><select class="inp" id="lgDriver"><option value="">-- Chọn tài xế --</option>${availDrivers.map(x=>`<option value="${e(x.id)}">${e(x.name)} · GPLX ${e(x.licenseClass)}</option>`).join('')}</select></div><div class="field"><label>Ngày/giờ xuất phát *</label><input class="inp" id="lgStart" type="datetime-local" value="${d.promisedDate}T08:00"></div><div class="field" style="grid-column:1/-1"><label>Tuyến giao</label><input class="inp" id="lgRoute" value="Kho thành phẩm → ${e(d.address)}"></div></div>`,foot:'<button class="btn" data-act="modal-close">Hủy</button><button class="btn btn-primary" data-act="lg-dispatch-save" data-id="'+e(d.id)+'"><i class="fa-solid fa-route"></i>Xác nhận điều phối</button>'});
  }
  function openVehicleForm(v=null) {
    Modal.open({title:v?'Cập nhật xe':'Thêm xe',size:'lg',body:`<div class="form-grid cols-2"><div class="field"><label>Biển số *</label><input class="inp" id="lgVPlate" value="${e(v?.plate||'')}"></div><div class="field"><label>Loại xe *</label><input class="inp" id="lgVType" value="${e(v?.type||'Xe tải lạnh')}"></div><div class="field"><label>Tải trọng (kg) *</label><input class="inp num" id="lgVCapacity" type="number" min="1" value="${n(v?.capacityKg)||1000}"></div><div class="field"><label>Nhiên liệu</label><select class="inp" id="lgVFuel"><option ${v?.fuelType==='Diesel'?'selected':''}>Diesel</option><option ${v?.fuelType==='Xăng'?'selected':''}>Xăng</option><option ${v?.fuelType==='Điện'?'selected':''}>Điện</option></select></div><div class="field"><label>Định mức (L/100km) *</label><input class="inp num" id="lgVNorm" type="number" step="0.1" min="0" value="${n(v?.fuelNorm)||10}"></div><div class="field"><label>Km hiện tại</label><input class="inp num" id="lgVOdo" type="number" min="0" value="${n(v?.odometer)}"></div><div class="field"><label>Thiết bị GPS</label><input class="inp" id="lgVGps" value="${e(v?.gpsDevice||'')}"></div><div class="field"><label>Hạn đăng kiểm</label><input class="inp" id="lgVReg" type="date" value="${e(v?.registrationExpiry||'')}"></div><div class="field"><label>Bảo trì kế tiếp</label><input class="inp" id="lgVNextDate" type="date" value="${e(v?.nextMaintenanceDate||addDaysLocal(today(),90))}"></div><div class="field"><label>Mốc km bảo trì</label><input class="inp num" id="lgVNextKm" type="number" min="0" value="${n(v?.nextMaintenanceKm)||5000}"></div></div>`,foot:`<button class="btn" data-act="modal-close">Hủy</button><button class="btn btn-primary" data-act="lg-vehicle-save" ${v?`data-id="${e(v.id)}"`:''}><i class="fa-solid fa-floppy-disk"></i>Lưu xe</button>`});
  }
  ...
  Actions['lg-delivery-save']=()=>{
    const orderId=document.querySelector('#lgOrderId')?.value||'', cargo=n(document.querySelector('#lgCargo')?.value), pd=document.querySelector('#lgPromiseDate')?.value, pt=document.querySelector('#lgPromiseTime')?.value;
  ...
  Actions['lg-delivery-view']=(d)=>{const x=delivery(d.id);if(!x)return;let buttons='';if(x.status==='WAIT_DISPATCH')buttons+=`<button class="btn btn-primary" data-act="lg-dispatch" data-id="${e(x.id)}"><i class="fa-solid fa-route"></i>Điều phối</button>`;if(x.status==='DISPATCHED'||x.status==='READY')buttons+=`<button class="btn btn-primary" data-act="lg-trip-start" data-id="${e(x.id)}"><i class="fa-solid fa-play"></i>Bắt đầu chuyến</button>`;if(x.status==='IN_TRANSIT')buttons+=`<button class="btn btn-primary" data-act="lg-trip-complete" data-id="${e(x.id)}"><i class="fa-solid fa-flag-checkered"></i>Xác nhận giao</button>`;if(['DELIVERED','PARTIAL'].includes(x.status))buttons+=`<button class="btn btn-primary" data-act="lg-trip-close" data-id="${e(x.id)}"><i class="fa-solid fa-receipt"></i>Chốt chuyến</button>`;Modal.open({title:`Chi tiết đơn giao · ${x.id}`,size:'xl',body:deliveryDetail(x),foot:`<button class="btn" data-act="modal-close">Đóng</button>${buttons}`});};
  Actions['lg-dispatch']=(d)=>{const x=delivery(d.id);if(x)openDispatch(x);};
  Actions['lg-dispatch-save']=(d)=>{const x=delivery(d.id),vid=document.querySelector('#lgVehicle')?.value,did=document.querySelector('#lgDriver')?.value,start=document.querySelector('#lgStart')?.value,cargo=n(document.querySelector('#lgDispatchCargo')?.value);if(!x||!vid||!did||!start||cargo<=0){Toast.err('Chưa đủ điều phối','Vui lòng nhập khối lượng, chọn xe, tài xế và thời gian xuất phát.');return;}const v=vehicle(vid),dr=driver(did);if(!v||v.status!=='AVAILABLE'||n(v.capacityKg)<cargo){Toast.err('Xe không khả dụng','Xe đang bận/bảo trì hoặc không đủ tải trọng.');return;}x.cargoKg=cargo;if(!dr||dr.status!=='AVAILABLE'){Toast.err('Tài xế không khả dụng','Tài xế đang chạy chuyến khác hoặc nghỉ.');return;}if(dr.licenseExpiry && dr.licenseExpiry<today()){Toast.err('GPLX hết hạn',`${dr.name} có GPLX đã hết hạn.`);return;}Object.assign(x,{vehicleId:vid,driverId:did,plannedStart:start,route:document.querySelector('#lgRoute')?.value.trim()||x.address,status:'DISPATCHED'});rerender('Đã điều phối',`${v.plate} · ${dr.name}`);};
  Actions['lg-trip-start']=(d)=>{const x=delivery(d.id);if(!x)return;x.status='IN_TRANSIT';x.actualStart=nowIso();x.gpsUpdatedAt=nowIso();const order=(DB.orders||[]).find(o=>o.id===x.orderId);if(order){order.status='dh_dang_giao';order.logisticsStatus='IN_TRANSIT';order.logisticsDeliveryId=x.id;SalesCRM?.saveLocal?.(['orders']);}rerender('Đã bắt đầu chuyến',`${x.id} đang giao hàng.`);};
  ...
  Actions['lg-trip-close']=(d)=>{const x=delivery(d.id);if(!x)return;const norm=normFuel(x);Modal.open({title:`Chốt chuyến · ${x.id}`,sub:`Định mức dự kiến ${num(norm,1)} L cho ${num(x.actualKm,1)} km`,size:'lg',body:`<div class="form-grid cols-2"><div class="field"><label>Nhiên liệu thực tế (L) *</label><input class="inp num" id="lgFuel" type="number" min="0" step="0.1" value="${num(norm,1)}"></div><div class="field"><label>Chi phí nhiên liệu *</label><input class="inp num" id="lgFuelCost" type="number" min="0" value="0"></div><div class="field"><label>Phí cầu đường</label><input class="inp num" id="lgToll" type="number" min="0" value="0"></div><div class="field"><label>Phí bãi xe</label><input class="inp num" id="lgParking" type="number" min="0" value="0"></div><div class="field"><label>Chi phí khác</label><input class="inp num" id="lgOther" type="number" min="0" value="0"></div></div>`,foot:`<button class="btn" data-act="modal-close">Hủy</button><button class="btn btn-primary" data-act="lg-trip-close-save" data-id="${e(x.id)}"><i class="fa-solid fa-lock"></i>Chốt chuyến</button>`});};
  Actions['lg-trip-close-save']=(d)=>{const x=delivery(d.id);if(!x)return;x.fuelLiters=n(document.querySelector('#lgFuel')?.value);x.fuelCost=n(document.querySelector('#lgFuelCost')?.value);x.tollCost=n(document.querySelector('#lgToll')?.value);x.parkingCost=n(document.querySelector('#lgParking')?.value);x.otherCost=n(document.querySelector('#lgOther')?.value);x.status='CLOSED';const order=(DB.orders||[]).find(o=>o.id===x.orderId);if(order){order.actualTransportCost=tripCost(x);order.transportCostPerKm=costPerKm(x);order.logisticsStatus='CLOSED';order.logisticsClosedAt=nowIso();SalesCRM?.saveLocal?.(['orders']);}rerender('Đã chốt chuyến',`Chi phí thực tế ${money(tripCost(x))} · ${money(costPerKm(x))}/km`);};
  ...
  Actions['lg-vehicle-save']=(d)=>{const plate=document.querySelector('#lgVPlate')?.value.trim(),type=document.querySelector('#lgVType')?.value.trim(),cap=n(document.querySelector('#lgVCapacity')?.value),norm=n(document.querySelector('#lgVNorm')?.value);if(!plate||!type||cap<=0||norm<0){Toast.err('Dữ liệu xe chưa hợp lệ','Kiểm tra biển số, loại xe, tải trọng và định mức.');return;}let v=d.id?vehicle(d.id):null;if(!v){v={id:`XE-${String(L.vehicles.length+1).padStart(3,'0')}`,status:'AVAILABLE'};L.vehicles.push(v);}Object.assign(v,{plate,type,capacityKg:cap,fuelType:document.querySelector('#lgVFuel')?.value||'Diesel',fuelNorm:norm,odometer:n(document.querySelector('#lgVOdo')?.value),gpsDevice:document.querySelector('#lgVGps')?.value.trim()||'',registrationExpiry:document.querySelector('#lgVReg')?.value||'',nextMaintenanceDate:document.querySelector('#lgVNextDate')?.value||'',nextMaintenanceKm:n(document.querySelector('#lgVNextKm')?.value)});rerender('Đã lưu xe',`${v.id} · ${v.plate}`);};
  ...
  function createFromSalesOrder(orderId, salesIssueId='') {
    ...
    const row = { id, orderId:order.id, salesIssueId:salesIssueId||order.salesIssueId||'', customer:snap.customer, address:snap.address, recipient:snap.recipient, phone:snap.phone, deliveryNote:snap.deliveryNote, shippingFee:snap.shippingFee, promisedDate:snap.promisedDate<today()?today():snap.promisedDate, promisedTime:'10:00', items:snap.items, plannedStart:'', actualStart:'', actualDelivered:'', vehicleId:'', driverId:'', route:'', estimatedKm:0, actualKm:0, cargoKg:0, deliveredQty:0, returnedQty:0, fuelLiters:0, fuelCost:0, tollCost:0, parkingCost:0, otherCost:0, status:'WAIT_DISPATCH', gpsLat:null, gpsLng:null, gpsUpdatedAt:'', note:snap.deliveryNote||`Tự động tạo sau khi Kho xuất đơn ${order.id}`, createdAt:nowIso(), autoCreated:true };
